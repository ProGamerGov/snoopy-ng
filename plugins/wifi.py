...
class Snoop(Thread):
    ...
    def get_parameter_list():
        ...
        info = {"info" : "This plugin intercepts and processes network traffic. A series of sub-plugins exists within the 'mods' subfolder. Existing sub-plugins are:%s"%sub_plugs,
                "parameter_list" : [("iface=<dev>", "interface to listen on. e.g. -m iface:iface=wlan0mon"),
                                    ("mon=[True|False]","First enable monitor mode on <iface>. e.g. -m wifi:iface=wlan0mon,mon=True. If no <iface> specified, will find first appropriate one."),
